...
export const actionController = {
  getAllActions: async (req: Request, res: Response) => {
    ...
    const message = result.length ? 'Actions fetched successfully' : 'No roles found'
  ...
  createAction: async (req: Request, res: Response) => {
    ...
    res.status(201).json({ success: true, message: 'Action created successfully', data: result })
  },
  updateAction: async (req: Request, res: Response) => {
    ...
    return res.status(200).json({ success: true, message: `Updated action with ID ${actionId}` })
  },
  deleteAction: async (req: Request, res: Response) => {
    ...
    if (!isHardDelete) {
      ...
      return res
        .status(200)
        .json({ success: true, message: `Soft deleted user with ID ${actionId}` })
    }

    await db.delete(actions).where(whereClause)

    return res.status(200).json({ success: true, message: `Deleted user with ID ${actionId}` })
  }
}
